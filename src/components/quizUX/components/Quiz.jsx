...
const Quiz = () => {
  ...
  // load of path graph when topics are loaded
  ...
  const handleCircleClick = (circle, isSectionStart = false) => {
    ...
    if (
      completedCircles.includes(circle) &&
      !isSectionStart &&
      state !== "REENTER"
    ) {
      ...
      if (circle !== lastClicked) {
        switch (state) {
          ...
          case "REENTER":
            ...
            setPossibleCircles(
              possibleCircles.filter((circle) => !circle.includes("circle"))
            );
            setState("DEFAULT");
            break;
          default:
  ...
  const handleAnswer = (isCorrect, usersAnswers) => {
    ...
    setCompletedCircles((completedCircles) => [
      ...completedCircles,
      lastClicked,
    ]);
  ...
  const handleUpdate = (szenario = null, szenarioTopic) => {
    ...
    if (!isExercise) {
      ...
      !completedCircles.includes(lastClicked) &&
        setCompletedCircles((completedCircles) => [
          ...completedCircles,
          lastClicked,
        ]);
  ...
  const handleTopicRepeat = (repeatTopic) => {
    ...
    if (completedAtLeastOnce)
      setPossibleCircles(
        possibleCircles.filter((circle) =>
          pathGraph[circle].topic.includes("szenario")
        )
      );
    ...
    setCompletedCircles(
      completedCircles.filter(
        (circle) => pathGraph[circle].topic !== repeatTopic
      )
    );
    setCorrectCircles(
      correctCircles.filter((circle) => pathGraph[circle].topic !== repeatTopic)
    );

    // update completed exercises by removing entries of circles that belong to repeated topic
    const updatedCompletedExercises = Object.fromEntries(
      Object.entries(completedExercises).map(([circle, value]) => {
        // look at number of circle (subsstring that comes after 'e') to reset depending on topic
        return parseInt(circle.substring(circle.indexOf("e") + 1)) >=
          (getTopicNumber(repeatTopic) - 1) * 8 + 1 &&
          parseInt(circle.substring(circle.indexOf("e") + 1)) <=
            getTopicNumber(repeatTopic) * 8
    ...
    const updatedJokerMap = Object.fromEntries(
      Object.entries(jokerMap).map(([circle, value]) => {
  ...
  const handleReviewClick = (direction) => {
    ...
    const numberCircle = parseInt(
      currCircle.substring(currCircle.indexOf("e") + 1)
    );
    // can't go left if circle is first or right if circle is last circle
    if (numberCircle === (direction === "left" ? 1 : 24)) return;
    // save new circle
    const newNumber =
      direction === "left" ? numberCircle - 1 : numberCircle + 1;
  ...
  const renderBoard = () => {
    ...
    return circles.map((circle) => {
      ...
      const isSzenario =
        topic === "szenario1" || topic === "szenario2" || topic === "szenario3";
      ...
      return (
        <g
          ...
          onMouseOver={() => handleCircleHover(circle)}
  ...
  return (
    <div className={`${styles.Quiz} overflow-hidden bg-[#545166]`}>
      <Header
        onReset={handleReset}
        doneInTopic={doneInTopic}
        correctInTopic={correctInTopic}></Header>
      <div className={styles.svg_container}>
        <svg id="board" viewBox="0 0 1470 820">
          ...
          <line
            ...
            x2={496}
            y2={1000}
            style={{ stroke: "#2D2C36", strokeWidth: "5px" }}
          />
          <line
            x1={992}
            y1={0}
      ...
      <div
        ...
        className={`${styles.help_btn} hover:opacity-80`}>
        <h2 className="cursor-pointer mb-0">?</h2>
      </div>
      {showPopup && (
        <Popup
